...
- **Clone repo** — `CloneRepoProvider` (section 2) shows one "Clone repo…" container whose children are the root's **known repos** (`RepoRegistry`, persisted per-root at `<root>/.lanes/repos.json`), so a fresh lane can search-and-clone a repo used before; `SubtreeIndex`'s child indexing makes them reachable by name from the lane's top level.
  The list is **auto-harvested**: `RepositoryProvider` records each discovered repo's raw `origin` URL (via `GitInspector.remoteURL`) through `RepoRegistry.harvest` (deduped by canonical `host/owner/slug`, ssh+https collapse), and a "Clone from URL…" input clones a pasted URL directly (no separate "add" step) — `performClone` calls `RepoRegistry.remember` so the just-cloned repo joins the list.
  `RepoCloner` clones into `<lane>/<slug>` (erroring if it exists) — **hybrid**: a `clone-repo` handler wins if configured (local `<root>/.lanes/config/clone-repo`, or a `clone-repo.catalog` pointer that beats the local file, resolved via `Catalogs.resolveExecutable` and swept by `removePointers`), run with `REPO_URL`/`REPO_NAME` + `LANE_*` (cwd = lane); otherwise a built-in `git clone`.
  On success the run returns `.enter(lane)` so the new repo lists.
