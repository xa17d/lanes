...
- **Lane** (`Model/`) — a folder is a lane.
  ...
  `LaneFS` exposes the well-known `<root>/.lanes/…` paths (`templateDir`, `templatePointer`, `scriptDir`, `repoScriptDir`, `hookDir`, `catalogDir`, `catalogCheckout`, `cloneScript`, `cloneScriptPointer`, `repoRegistryURL`).
...
- **Script items** (`Providers/ScriptItems.swift`) — turns executable files (and resolved `.catalog` pointers) under `script/` into `BasicItem` actions via `ScriptItems.effectiveScripts`.
  ...
  Scripts run **silently** via `Shell.run` with the lane (or repo) dir as cwd and `LANE_DIR`/`LANE_NAME`/`LANE_ID` (+ `TICKET_KEY`/`TICKET_URL` for the lane's first linked ticket and `TICKET_KEYS`/`TICKET_URLS` for all of them newline-separated and index-aligned, resolved via `TicketProvider.env` and omitted when none is linked; + `REPO_DIR`/`REPO_NAME` for repo scripts) in the environment; a nonzero exit throws and surfaces stderr as an error toast.
...
- **Clone repo** — `CloneRepoProvider` (section 2) shows one "Clone repo…" container whose children are the root's **known repos** (`RepoRegistry`, persisted per-root at `<root>/.lanes/repos.json`), so a fresh lane can search-and-clone a repo used before; `SubtreeIndex`'s child indexing makes them reachable by name from the lane's top level.
  ...
  Both the known-repo rows and the query fallback return `RunOutcome.startClone(lane:url:)`; provider items stay declarative and **all clone execution lives in `LaneModel`**.
  `LaneModel.startClone` runs the clone **in the background** (detached; the panel is never blocked and several can run at once): it registers a `CloneJob` in `activeClones`, and `rows` overlays a spinner `DisplayRow` (`isLoading`) per job at the lane's top level (hidden once the real repo surfaces). `honor(.startClone)` also `enter(lane)`s so you're returned to the lane immediately. On completion `finishClone` drops the job, `RepoRegistry.remember`s the repo (so it joins the list), surfaces any warning/error toast, and `reloadCurrentInPlace`s if that lane is on screen.
  `RepoCloner` clones into `<lane>/<slug>` (erroring if it exists) — **hybrid**: a `clone-repo` handler wins if configured (local `<root>/.lanes/config/clone-repo`, or a `clone-repo.catalog` pointer that beats the local file, resolved via `Catalogs.resolveExecutable` and swept by `removePointers`), run with `REPO_URL`/`REPO_NAME` + `LANE_*` (cwd = lane); otherwise a built-in `git clone`. A non-zero exit that still produced the repo is `.clonedWithWarnings` (surfaced, not fatal).
  Well-known paths: `LaneFS.repoRegistryURL`, `cloneScript`, `cloneScriptPointer`.
