...
- **Catalogs** (`Services/Catalogs.swift`) — git repos of shared config a root subscribes to (managed in Settings).
  ...
  Local config references a catalog item through a thin `.catalog` **pointer** file (`{catalog, item}`) whose *own* filename supplies the display order/icon/name; `Catalogs.resolvePointer` (pure, no Shell) maps it to `catalog/<id>/checkout/<item>`.
  ...
  The git lifecycle (`add`/`fetch`/`apply`/`remove`, all needing a `Shell`) is split from resolution so non-Shell callers (template seeding) can still resolve pointers; **fetch never changes the checkout** — only the explicit `apply` advances `pin`.
  `AppDelegate` fetches stale catalogs in the background (~daily + hourly timer) and shows a menu-bar dot when `Catalogs.anyUpdatesAvailable`.
